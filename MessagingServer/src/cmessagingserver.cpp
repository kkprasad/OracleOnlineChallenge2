/*******************************************************************************
File Name               : cmessagingserver.cpp
Class Name              : CMessagingServer
Description             : This file contains the methods of the class 
			  CMessagingServer which is inherited from CServerSocket
			  This class handles the message processing task
*******************************************************************************/
...
long int CMessagingServer::m_lPacketNumber = 0;
...
void CMessagingServer::OnMessageReceive(std::string a_sReceivedMessage,int a_iReceivedMessageLength)
{
	...
	try
	{
		CFileHandler::GetInstance().WriteToFile(900,"REQUEST:PacketNumber:%d:SockeFD:%d:%s", ++CMessagingServer::m_lPacketNumber,this->GetSocketInstance()->GetSocketFD(),a_sReceivedMessage.c_str());	
...
void CMessagingServer::OnAccept(int a_iSocketFD)
{
	...
	try
	{
		...
		CFileHandler::GetInstance().WriteToFile(400,"Receive Thread started for socket FD:%d",a_iSocketFD);	
	...
	while(CMessagingServer::GetServerRunningStatus())
	{
		...
		if(CMessagingServer::m_qObseleteMessageHandlerQueue.size()>0)
		{
			...
			if(CMessagingServer::m_mActiveMessagingServerList.find(l_iSocketFD) != CMessagingServer::m_mActiveMessagingServerList.end())
			{
				...
				CFileHandler::GetInstance().WriteToFile(400,"Removed object from ActiveMessagingServerList:SocketFD=%d",l_iSocketFD);	

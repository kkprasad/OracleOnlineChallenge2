/*******************************************************************************
File Name               : messagingservermain.cpp
Main Function Name      : main
Description             : This file contains the procedures for using 
			  CMessagingServer class object and run the 
			  applicaiton as a messaging server
*******************************************************************************/
...
int main(int argc, char* argv[])
{
	...
	{
		CFileHandler::GetInstance().WriteToSysLog("Environment variable MSGSERVER_HOME is not set. ");
//		exit(EXIT_FAILURE);
	}
	...
	try
	{
		if(l_uconfigData.GetConfigurationFileName().length() == 0)
		{
			if(l_sHomeDirectory.length() != 0)
			{
				g_sConfigurationFileName = l_sHomeDirectory;
				g_sConfigurationFileName += "/etc/msgserverd.conf";
				l_uconfigData.SetConfigurationFileName(g_sConfigurationFileName);
			}
		}
		if(l_uconfigData.GetConfigurationFileName().length() >0 )
		...
		{
			CFileHandler::GetInstance().WriteToSysLog("The configuration file name is not set");
		}
	}
	...
	try
	{
		...
		else if(l_uconfigData.GetLogFileDirectory().length() == 0)
		{
			...
			{
				g_sLogDirectory = l_sHomeDirectory;
				g_sLogDirectory += "/var/log";	
				l_uconfigData.SetLogFileDirectory(g_sLogDirectory);
			}
		}
	}
	...
	try
	{
		...
		if (!g_upMessagingServer->StartServer())
		{
			CFileHandler::GetInstance().WriteToSysLog("StartServer Returned FALSE. Exiting");
			CFileHandler::GetInstance().WriteToFile(400,"StartServer Returned FALSE. Exiting...\n");
		...
		if(!g_upMessagingServer->StartThreads())
		{
			CFileHandler::GetInstance().WriteToSysLog("StartThreads Returned FALSE. Exiting");
			CFileHandler::GetInstance().WriteToFile(400,"StartThreads Returned FALSE. Exiting...\n");
			ExecuteExitProcedures(EXIT_FAILURE);
